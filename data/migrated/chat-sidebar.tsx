      ...
      return (
        <TooltipProvider>
          <TooltipRoot>
            <TooltipTrigger
              render={
                <a href={item.to} onClick={handleSelect}
                  className={cls}
...
function SectionLabel({
  ...
  if (collapsible) {
    return (
      ...
      >
        {navigateTo ? (
          <a href={navigateTo} className="text-[10px] font-semibold uppercase tracking-wider text-primary-500 dark:text-neutral-400 hover:text-primary-700 dark:hover:text-neutral-200 select-none transition-colors"
          >
            {label}
          </a>
        ) : (
    ...
    >
      ...
      {!isVisuallyCollapsed &&
        ...
        <div className="px-2 pb-2">
          <a href="/playground" onClick={() => onSelectSession?.()}
            className={cn(
              buttonVariants({ variant: 'ghost', size: 'sm' }),
