...
function ChatComposerComponent({
  ...
  const [modelsData, setModelsData] = useState<Awaited<ReturnType<typeof fetchModels>> | undefined>(undefined)
  const [modelsLoading, setModelsLoading] = useState(false)
  useEffect(() => {
    let mounted = true
    setModelsLoading(true)
    fetchModels().then(d => { if (mounted) { setModelsData(d); setModelsLoading(false) } }).catch(() => setModelsLoading(false))
    const id = setInterval(() => { fetchModels().then(d => { if (mounted) setModelsData(d) }) }, 60_000)
    return () => { mounted = false; clearInterval(id) }
  }, [])
  const modelsQuery = { data: modelsData, isLoading: modelsLoading }
  ...
  const [otherProviderModelsData, setOtherProviderModelsData] = useState<Record<string, Array<ModelCatalogEntry>> | undefined>(undefined)
  useEffect(() => {
    if (!isProviderSwitcherExpanded || otherProviders.length === 0) return
    let mounted = true
    const fetchOther = async () => {
      ...
      if (mounted) {
        setOtherProviderModelsData(modelEntries.reduce<Record<string, Array<ModelCatalogEntry>>>(
          (acc, entry) => { acc[entry.providerId] = entry.models; return acc }, {},
        ))
      }
    }
    fetchOther()
    return () => { mounted = false }
  }, [isProviderSwitcherExpanded, otherProviders])
  const otherProviderModelsQuery = { data: otherProviderModelsData, isLoading: false }
  const [currentModelData, setCurrentModelData] = useState<unknown>(undefined)
  const [currentModelLoading, setCurrentModelLoading] = useState(false)
  useEffect(() => {
    let mounted = true
    setCurrentModelLoading(true)
    fetchCurrentModelFromStatus().then(d => { if (mounted) { setCurrentModelData(d); setCurrentModelLoading(false) } }).catch(() => setCurrentModelLoading(false))
    const id = setInterval(() => { fetchCurrentModelFromStatus().then(d => { if (mounted) setCurrentModelData(d) }) }, 30_000)
    return () => { mounted = false; clearInterval(id) }
  }, [])
  const currentModelQuery = { data: currentModelData, isLoading: currentModelLoading }
  const [gatewayModeData, setGatewayModeData] = useState<unknown>(undefined)
  useEffect(() => {
    let mounted = true
    fetchGatewayMode().then(d => { if (mounted) setGatewayModeData(d) }).catch(() => {})
    return () => { mounted = false }
  }, [])
  const gatewayModeQuery = { data: gatewayModeData, isLoading: false }
  const [modelInfoData, setModelInfoData] = useState<unknown>(undefined)
  useEffect(() => {
    let mounted = true
    fetchModelInfo().then(d => { if (mounted) setModelInfoData(d) }).catch(() => {})
    return () => { mounted = false }
  }, [])
  const modelInfoQuery = { data: modelInfoData, isLoading: false }
  const zeroForkModelInfoFlags = useMemo(
    () => getZeroForkModelInfoFlags(modelInfoQuery.data),
    [modelInfoQuery.data],
  )

  const [profilesData, setProfilesData] = useState<unknown>(undefined)
  useEffect(() => {
    let mounted = true
    fetchProfiles().then(d => { if (mounted) setProfilesData(d) }).catch(() => {})
    return () => { mounted = false }
  }, [])
  const profilesQuery = { data: profilesData, isLoading: false }
  const [workspaceContextData, setWorkspaceContextData] = useState<unknown>(undefined)
  useEffect(() => {
    let mounted = true
    fetchWorkspaceContext().then(d => { if (mounted) setWorkspaceContextData(d) }).catch(() => {})
    return () => { mounted = false }
  }, [])
  const workspaceContextQuery = { data: workspaceContextData, isLoading: false }
  const activateProfileAction = async (profileName: string) => {
    try {
      await activateProfile(profileName)
      // Refetch data
      fetchProfiles().then(d => setProfilesData(d)).catch(() => {})
      fetchWorkspaceContext().then(d => setWorkspaceContextData(d)).catch(() => {})
      fetchModels().then(d => setModelsData(d)).catch(() => {})
      fetchCurrentModelFromStatus().then(d => setCurrentModelData(d)).catch(() => {})
      setIsProfileMenuOpen(false)
      toast(`Activated profile ${profileName}`)
    } catch (error) {
      toast(error instanceof Error ? error.message : 'Failed to activate profile')
    }
  }
  const selectWorkspaceAction = async (workspace: { path: string; name?: string }) => {
    try {
      const response = await fetch('/api/workspace', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      fetchWorkspaceContext().then(d => setWorkspaceContextData(d)).catch(() => {})
      toast(`Selected workspace ${workspace.name || workspace.path}`)
    } catch (error) {
      toast(error instanceof Error ? error.message : 'Failed to select workspace')
    }
  })
        ...
        {isMobileViewport ? (
          ...
          <>
            <div className="flex items-center gap-2 px-3 py-2">
              ...
              >
                <Add01Icon className="size-18 shrink-0" strokeWidth={1.5} />
              </button>
              ...
              <div className="shrink-0">
                {isLoading ? (
                  ...
                  >
                    <StopIcon className="size-18 shrink-0" strokeWidth={2} />
                  </button>
                ) : value.trim().length > 0 ||
                  attachments.length > 0 ||
                  attachmentProcessingCount > 0 ? (
                ...
                {!hideModelSelector ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <ArrowDown01Icon className="size-11 shrink-0" />
                    </button>
                    {isControlsMenuOpen ? (
      ...
      {previewImage &&
        createPortal(
          ...
          >
            ...
            >
              <Cancel01Icon className="size-24 shrink-0" strokeWidth={2} />
            </button>
            <img
...
export default MemoizedChatComposer
export type {
