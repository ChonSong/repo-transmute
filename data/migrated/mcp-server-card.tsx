...
import { isArgPlaceholder, isUrlPlaceholder } from '@/screens/mcp/lib/placeholder-detect'
...
function useTestMcpServer() {
  ...
  const mutate = useCallback(async (input: { name: string }) => {
    setIsPending(true)
    try {
      const res = await fetch(`/api/mcp/${encodeURIComponent(input.name)}/test`)
  ...
  return { isPending, mutate, mutateAsync: mutate }
}

function useConfigureMcpServer() {
  const [isPending, setIsPending] = useState(false)

  const mutate = useCallback(async (input: { name: string; enabled: boolean }) => {
    setIsPending(true)
    try {
      const res = await fetch(`/api/mcp/${encodeURIComponent(input.name)}`, {
        method: 'PATCH',
...
function useMcpOauth(): UseMcpOauthReturn {
  const [isPending, setIsPending] = useState(false)
  const [isError, setIsError] = useState(false)
  const [error, setError] = useState<Error | null>(null)
  const [data, setData] = useState<{ status: string } | null>(null)

  const start = useCallback(async (server: McpServer) => {
    setIsPending(true)
    setIsError(false)
    setError(null)
    try {
      const res = await fetch(`/api/mcp/${encodeURIComponent(server.name)}/oauth/reauth`, {
        method: 'POST',
      })
      if (!res.ok) throw new Error(`Reauth failed: ${res.status}`)
  ...
  return { isPending, isError, error, data, start }
}

export default function McpServerCard({ server, onEdit }: Props) {
  ...
  const liveOnlyTitle = fallbackMode
    ? 'Requires runtime /api/mcp endpoint (not available in local fallback mode).'
    : ''
