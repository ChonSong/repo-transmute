...
interface OperationsAgentDetailProps {
  open: boolean
...
function ModelSelector({
  ...
  const selected = (() => {
    ...
    return {
      id: value,
      provider: slashIdx > 0 ? value.slice(0, slashIdx) : 'model',
...
export default function OperationsAgentDetail({
  open,
  agent,
  onClose,
  onSave,
  ...
  useEffect(() => {
    ...
    fetchModels()
      ...
      .catch(() => {
        // Silently fail — model list is optional
      })
      .finally(() => {
    ...
    >
      ...
      >
        ...
        <label className="mt-4 block space-y-2">
          ...
          {isModelsLoading && (
            <p className="mt-1 text-xs text-[var(--theme-muted)]">Loading models…</p>
          )}
        </label>
