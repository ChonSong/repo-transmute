...
// --- Standalone Dialog implementation (replaces @base-ui/react) ---
...
function DialogTitle({ className, children }: DialogTitleProps) {
  return <h2 className={cn('text-lg font-semibold text-primary-900', className)}>{children}</h2>
}
...
function DialogDescription({ className, children }: DialogDescriptionProps) {
  return <p className={cn('text-sm text-primary-600', className)}>{children}</p>
}
...
export default function ProviderWizard({
  ...
  async function handleSaveApiKey() {
    ...
    try {
      ...
      // Trigger restart — refresh page to apply changes
