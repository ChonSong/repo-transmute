...
interface OperationsInlineChatProps {
  agentName: string
...
interface OperationsAgentCardProps {
...
export default function OperationsAgentCard({
  agent,
  onOpenSettings,
}: OperationsAgentCardProps) {
  ...
  async function handleToggleCron(jobId: string, enabled: boolean) {
    ...
    try {
      ...
      toast(
        e instanceof Error ? e.message : 'Failed to update cron job',
        { type: 'error' },
      )
    } finally {
  ...
  async function handleRunCron(jobId: string) {
    ...
    try {
      ...
      toast(
        e instanceof Error ? e.message : 'Failed to run cron job',
        { type: 'error' },
      )
    } finally {
    ...
    <article className="flex min-h-[19rem] flex-col rounded-[1.5rem] border border-[var(--theme-border)] bg-[var(--theme-card)] p-3 shadow-[0_20px_60px_color-mix(in_srgb,var(--theme-shadow)_14%,transparent)]">
      <div className="relative flex min-h-8 items-center">
        ...
        <div className="absolute right-0 flex items-center gap-1">
          ...
          >
            {isActive ? <Pause size={16} strokeWidth={1.8} /> : <Play size={16} strokeWidth={1.8} />}
          </button>
              ...
              {agent.jobs.length > 0 ? (
                <>
                      ...
                      >
                        <label className="relative inline-flex cursor-pointer items-center">
                          <input
                            ...
                            checked={job.enabled}
                            onChange={() =>
                              void handleToggleCron(job.id, !job.enabled)
                            }
                  ...
                  <div className="mt-3 flex justify-end">
                    <a
                      href="/jobs"
                      className="inline-flex h-8 items-center rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg)] px-3 text-xs font-medium text-[var(--theme-text)] transition-colors hover:bg-[var(--theme-card2)]"
                    >
                      + Add Job
                    </a>
                  </div>
